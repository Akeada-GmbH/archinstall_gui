    ...
    if '_module' in frame.data and frame.data['_module'] == 'installation_steps/finalisieren':
        ...
        if not 'disk_password' in frame.data:
            ...
            if os.path.exists("/boot/.success"):
                ...
                for c in ["bash /usr/share/privastick/scripts/PrivaStickResize", "sed -i 's/i3/xfce/g' /etc/lightdm/lightdm.conf", "sed -i 's/privauser//g' /etc/lightdm/lightdm.conf", "sed -i 's/NOPASSWD: ALL/NOPASSWD: \/usr\/share\/privastick\/scripts\/PrivaStickVPNSetup, \/usr\/share\/privastick\/scripts\/misc\/install-app, \/usr\/share\/privastick\/scripts\/misc\/remove-app/g' /etc/sudoers", "passwd -l root", "overlay_flush", "systemctl restart lightdm"]:
